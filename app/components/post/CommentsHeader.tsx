...
const CommentsHeader = ({ post, params }: CommentsHeaderCompTypes) => {
  ...
  const hasUserLikedPost = async () => {
    ...
    let res = await useIsLiked(contextUser.user.id, params.postId, likesByPost);
        ...
        <div className="flex items-center">
          ...
          <div className="ml-3 pt-0.5">
            ...
            <div className="relative z-0 text-[13px] -mt-5 font-light">
              ...
              <span className="font-medium">{moment(post?.created_at).format('YYYY-MM-DD HH:mm')}</span>
            </div>
          </div>
        </div>
        ...
        <ClientOnly>
          <div className="pb-4 text-center flex items-center">
            <button
              ...
              className={`
                rounded-full bg-gray-200 p-2 cursor-pointer transition-colors duration-300 ease-in
                ${likesByPost.length > 0 && userLiked ? 'bg-[#ff2626] bg-opacity-10 hover:bg-gray-200 hover:bg-opacity-1' : ''}
              `}
            >
              {!hasClickedLike ? (
