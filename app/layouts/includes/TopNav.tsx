        ...
        <div className={`flex items-center justify-between gap-6 w-full px-4 mx-auto ${pathName == '/' ? 'max-w-[1150px]' : ''}`}>
          ...
          <div className='relative hidden md:flex items-center justify-end bg-[#F1F1F2] p-1 rounded-full max-w-[430px] w-full'>
            ...
            {searchProfiles.length > 0 ? (
              <>
                <div className='absolute bg-white max-w-[910px] h-auto w-full z-20 left-0 top-12 border p-0'>
                  {searchProfiles.map((profile, index) => (
                    <div className="p-0" key={index}>
                      <Link
                        href={`/profile/${profile?.id}`}
                        className='flex items-center justify-between w-full py-1 cursor-pointer hover:bg-[#F12B56] px-2 hover:text-white'
                      >
                        <div className="flex items-center">
                          <img className='rounded-md' width="40" src={userCreateBucketUrl(profile?.image)} />
              ...
              <div className="flex items-center">
                <div className="relative">
                  <button
                    ...
                    <img className='rounded-full w-[35px] h-[35px]' src={userCreateBucketUrl(userContext?.user?.image || '')} />
                  </button>
                  {showMenu ? (
